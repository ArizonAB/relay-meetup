...
let make = () =>
  <footer
    className="mt-16 md:mt-20 md:pb-32 pt-16 justify-center bg-gray-900 text-white">
    <div className="md:pb-64 pb-32">
      <div
        className="flex flex-row justify-center items-center flex-wrap px-6">
        {footerItems
         ->Belt.Array.mapWithIndex((index, {text, link}) =>
             <div key=link className={index == 0 ? "my-2" : "my-2 ml-6"}>
               <Next.Link href=link>
                 <a className=""> {React.string(text)} </a>
               </Next.Link>
             </div>
           )
         ->React.array}
    ...
    <div className="text-xs text-center pt-16 pb-6">
      ...
      <a
        href="https://www.flaticon.com/free-icon/nodes_2165534"
        title="Vitaly Gorbachev">
        {React.string("Vitaly Gorbachev")}
      </a>
      {React.string(" from ")}
      <a href="https://www.flaticon.com/" title="Flaticon">
        {React.string("www.flaticon.com")}
      </a>
    </div>
  </footer>;
