let dateTimestampFromDate = (date: Types.date) =>
  ...
  ("T" ++
  (Util.toTwoDigitStr(date.hour) ++ (":" ++ (Util.toTwoDigitStr(date.minute) ++ "+02:00"))))))))
let getEventMarkup = (meetup: Types.meetupEvent) => {
  let startDate = dateTimestampFromDate(meetup.date)
  let endDate = dateTimestampFromDate({...meetup.date, hour: meetup.date.hour + 2})

  {
    ...
    ->Belt.Array.map(p => {"@type": "Person", "name": p.name, "jobTitle": p.work}),
    "organizer": [
      {
        "@type": "Organization",
        "name": "Relay Meetup",
...
let make = (~meetup: Types.meetupEvent) =>
  <script
    ...
    dangerouslySetInnerHTML={
      "__html": meetup->getEventMarkup->Js.Json.stringifyAny->Belt.Option.getWithDefault(""),
    }
  />
